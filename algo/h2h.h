...
class H2H : public Ch {
 ...
 private:
  ...
  //  refer to  M. A. Bender and M. Farach-Colton. The lca problem revisited. In
  //  Proc. of LASTI’00, pages 88–94, 2000
  void build_rmq();
  void rmq_dfs(int p);
...
void H2H::processing() { remained_neigh.resize(v_size_); }
...
int H2H::get_lca(int _p, int _q) {
  ...
  q = q - i + 1;
  if (tree_.nodes[tree_.RMQIndex[k][p]].height <
      tree_.nodes[tree_.RMQIndex[k][q]].height)
